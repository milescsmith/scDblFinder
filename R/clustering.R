...
.prepSCE <- function(sce, ndims = 30, nfeatures = 1000, ...) {
  if (!("logcounts" %in% assayNames(sce))) {
    if (is.null(librarySizeFactors(sce))) sce <- computeLibraryFactors(sce)
    ls <- librarySizeFactors(sce)
    if (any(is.na(ls) | ls == 0))
      stop(
        "Some of the size factors are invalid. Consider removing",
  ...
  if (!("PCA" %in% reducedDimNames(sce))) {
    sce <- runPCA(
      sce,
      ncomponents = ifelse(is.null(ndims), 30, ndims),
