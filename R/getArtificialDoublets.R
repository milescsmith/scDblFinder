...
#'
#' @export
getArtificialDoublets <- function(
  x,
  n = 3000,
  clusters = NULL,
  resamp = 0.25,
  ...
  if (is.null(clusters)) {
    w <- which(
      ls > 0 & ls >= quantile(ls, min(trim.q)) & ls <= quantile(ls, max(trim.q))
    )
  } else {
    clo <- clusters
    ...
    w <- which(
      ls > 0 & ls >= q[1, clusters] & ls <= q[2, clusters] & !is.na(clusters)
    )
  ...
  if (is.null(clusters) || propRandom == 1) {
    ...
    if (ncol(x)^2 <= n) {
      ...
      ad <- matrix(
        sample.int(ncol(x), 2 * n, replace = (2 * n >= ncol(x))),
        ncol = 2
      )
    }
  ...
  ca <- getCellPairs(
    clo,
    n = ifelse(n.meta.cells > 0, ceiling(n * 0.9), n),
    selMode = selMode
  )
  m2 <- createDoublets(
    x,
  ...
  if (meta.triplets && sum(tt >= pc10) > 2) {
    ...
    m2 <- round(
      (meta[, ca[, 1], drop = FALSE] + meta[, ca[, 2]] + meta[, ca[, 3]]) / 2
    )
...
getCellPairs <- function(
  ...
  ca <- do.call(
    rbind,
    lapply(seq_len(nrow(ca)), FUN = function(i) {
      cbind(
        cell1 = sample(
          cli[[ca[i, 1]]],
          size = ca[i, 4],
...
.getMetaCells <- function(x, clusters, n.meta.cells = 20, meta.cell.size = 20) {
  ...
  meta <- vapply(
    unlist(meta, recursive = FALSE),
    FUN.VALUE = double(nrow(x)),
    FUN = function(y) {
      Matrix::rowMeans(x[, y, drop = FALSE])
    }
  )
...
addDoublets <- function(
  ...
  ca <- getCellPairs(
    clusters,
    n = length(ed) * (30 + max(ed)),
    ls = Matrix::colSums(x)
  )
  ca <- split(ca, ca$orig.clusters)
  ca <- do.call(
    rbind,
    lapply(names(ed), FUN = function(i) {
      ca2 <- ca[[i]]
  ...
  cd <- data.frame(
    type = rep(c("singlet", "doublet"), c(ncol(x), ncol(m2))),
    cluster = cl
  )
...
createDoublets <- function(
  ...
  x1 <- x[, dbl.idx[wNad, 1], drop = FALSE] +
    x[, dbl.idx[wNad, 2], drop = FALSE]
  if (length(wAd) > 1) {
