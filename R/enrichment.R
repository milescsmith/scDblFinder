...
clusterStickiness <- function(
  ...
  if (is(x, "SingleCellExperiment")) {
    ...
    if (is.null(x))
      stop(
        "No doublet origin statistics; was scDblFinder run ",
...
doubletPairwiseEnrichment <- function(
  ...
  if ("difficulty" %in% colnames(x) && inclDiff) {
    ...
    mod <- glm(
      x$observed ~ 0 + offset(log(x$expected)) + log(x$difficulty),
      family = theta
    )
  ...
  if (!sampleWise && "sample" %in% colnames(x))
    x <- aggregate(
      x[, setdiff(colnames(x), c("sample", "combination"))],
      by = x[, "combination", drop = FALSE],
      FUN = sum
    )
  if (type == "binomial") {
    p <- pbinom(
      x$observed,
      prob = x$expected / sum(x$expected),
      size = sum(x$observed),
      lower.tail = FALSE
    )
  } else {
...
.getThetaDist <- function(
  ...
  if (is(theta, "try-error")) {
    if (verbose)
      warning(
        "Not enough dispersion (theta diverges to infinity) ",
