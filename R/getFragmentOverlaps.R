...
#' @export
getFragmentOverlaps <- function(
  x,
  barcodes = NULL,
  regionsToExclude = GRanges(
  ...
  if (ret == "coverages" && !fullInMemory)
    stop(
      "Returning coverages is currently only supported with fullInMemory=TRUE"
    )
  ...
  if (!is.null(regionsToExclude)) {
    if (
      is.character(regionsToExclude) &&
        length(regionsToExclude) == 1 &&
        file.exists(regionsToExclude)
    ) {
      if (verbose)
        message(format(Sys.time(), "%X"), " - Reading regions to exclude")
  ...
  if (is.character(x) && length(x) == 1) {
    ...
    if (
      ...
      x <- bplapply(seqnamesTabix(tf), BPPARAM = BPPARAM, FUN = function(x) {
        ...
        getFragmentOverlaps(
          ...
          verbose = FALSE,
          minFrags = 0.00001,
          uniqueFrags = uniqueFrags,
          maxFragSize = maxFragSize,
          removeHighOverlapSites = removeHighOverlapSites,
          BPPARAM = NULL,
          ret = ret
        )
      })
      ...
      if (!fullInMemory) {
        message(
          "Fragment file is not tabix-indexed, requiring the",
    ...
    if (!is(x, "GRanges") || !("name" %in% colnames(mcols(x))))
      stop(
        "`x` should either be a path to a fragments file, or a GRanges ",
  ...
  if (!all(!is.na(seqlengths(gr))))
    seqlengths(gr) <- setNames(
      sapply(split(end(gr), seqnames(gr)), max)[seqlevels(gr)],
      seqlevels(gr)
    )
  ...
  if (!is.null(regionsToExclude)) {
    ...
    if (length(regionsToExclude) > 0) {
      regionsToExclude <- keepSeqlevels(
        regionsToExclude,
  ...
  if (verbose)
    message(format(Sys.time(), "%X"), " - Splitting and subsetting barcodes...")
  ...
  grl <- GRangesList(lapply(gr, FUN = function(x) {
    ...
    GRanges(
      rep(factor(names(x), seqlevels(gr)), lengths(x)),
...
.removeHighOverlapSites <- function(
  gr,
  pthres = 0.01,
  retExclusionRanges = FALSE
) {
