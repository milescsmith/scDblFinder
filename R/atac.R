...
clamulet <- function(
  ...
  m <- .clamulet.buildTable(
    x,
    nADbls = artificialDoublets,
    maxN = maxN,
    nfeatures = nfeatures,
    verbose = verbose,
    ...
...
.clamulet.buildTable <- function(
  ...
  if (minCount < 1)
    minCount <- max(as.integer(round(minCount * length(co))), 2L)
  ...
  windows <- reduce(
    .sliceGR(windows, lower = minCount, seqlvls = seqlvls),
    min.gapwidth = 100L
  )
  ...
  comb <- matrix(
    sample.int(length(co), size = 2 * floor(nADbls * 2.2), replace = TRUE),
    ncol = 2
  )
  ...
  if (verbose)
    message(format(Sys.time(), "%X"), " - Counting overlaps for real cells")
  d <- data.frame(
    row.names = c(colnames(counts), colnames(acounts)),
    type = rep(
      factor(c("real", "doublet"), c("real", "doublet")),
  ...
  co <- lapply(
    setNames(seq_len(nrow(comb)), colnames(acounts)),
    FUN = function(x) {
      x <- comb[x, ]
...
.sumRleLists <- function(x, y) {
  ...
  as(
    lapply(useqlvls, FUN = function(seql) {
...
.sliceGR <- function(x, lower = 1L, seqlvls = NULL) {
  ...
  GRanges(
    rep(factor(names(x), seqlvls), lengths(x)),
...
amuletFromCounts <- function(
  ...
  q <- p.adjust(
    ppois(d$nAbove2, lambda = mean(d$nAbove2), lower.tail = FALSE),
    method = "fdr"
  )
