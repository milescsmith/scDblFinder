...
doubletThresholding <- function(
  ...
  w <- vapply(
    conds,
    FUN.VALUE = logical(1),
    FUN = function(x) all(x %in% colnames(d))
  )
  if (method == "auto") {
    ...
    if (!is.null(d$sample) && perSample) {
      ...
      th <- sapply(setNames(names(si), names(si)), FUN = function(s) {
        .optimThreshold(
          d[
            si[[s]],
            c("type", "src", "score", "cluster", "include.in.training")
          ],
          dbr = dbr[[s]],
          dbr.sd = dbr.sd,
...
.optimThreshold <- function(
  ...
  totfn <- function(x) {
    ...
    if (!is.null(fdr.include))
      y <- y +
        .FPR(d$type[fdr.include], d$score[fdr.include], x) * 2 * stringency
    y
  }
...
.compareToExpectedDoublets <- function(
  origins,
  expected,
  dbr = NULL,
  od = NULL
...
.getDoubletStats <- function(
  ...
  if (!is.null(d$sample))
    return(dplyr::bind_rows(
      lapply(split(seq_len(nrow(d)), d$sample), FUN = function(i) {
  ...
  stats$FNR <- vapply(
    split(as.data.frame(d), d$mostLikelyOrigin),
    FUN.VALUE = numeric(1L),
    FUN = function(x) .FNR(x$type, x$score, th)
  )[stats$combination]
  stats$difficulty <- vapply(
    split(d$difficulty, d$mostLikelyOrigin),
    FUN.VALUE = numeric(1L),
    na.rm = TRUE,
    FUN = median
  )[stats$combination]
...
.filterUnrecognizableDoublets <- function(d, minSize = 5, minMedDiff = 0.1) {
  if (is.null(d$src)) d$src <- d$type
  da <- d[
    d$src == "artificial" & grepl("+", d$mostLikelyOrigin, fixed = TRUE),
  ]
  dr <- d[d$src == "real", ]
...
.estimateHeterotypicDbRate <- function(d, dbr = NULL) {
  ...
  if (is.null(d$cluster)) {
    ...
    prop.homo <- sum(d$src == "artificial" & d$score < th) /
      sum(d$src == "artificial")
