...
getExpectedDoublets <- function(
  ...
  expected <- data.frame(
    type1 = rep(seq_along(lvls), ncol(expected)),
...
.castorigins <- function(e, val = NULL) {
  if (is.table(e) || is.null(dim(e))) {
    e <- cbind(
      do.call(rbind, strsplit(names(e), "+", fixed = TRUE)),
...
selFeatures <- function(
  ...
  if ((ng <- ceiling((1 - propMarkers) * nfeatures)) > 0) {
    if (is.null(clusters)) {
      g <- row.names(sce)[order(
        Matrix::rowMeans(counts(sce), na.rm = TRUE),
        decreasing = TRUE
      )[seq_len(ng)]]
    } else {
      g <- tryCatch(
        {
          cl.means <- as.matrix(assay(scuttle::sumCountsAcrossCells(
            counts(sce),
            clusters
          )))
...
mockDoubletSCE <- function(
  ...
  if (is.null(names(ncells)))
    names(ncells) <- paste0("cluster", seq_along(ncells))
  if (is.null(mus)) {
    mus <- lapply(ncells, FUN = function(x) 2^rnorm(ngenes))
  } else {
    if (
      !is.list(mus) ||
        length(mus) != length(ncells) ||
        length(unique(lengths(mus))) != 1
    )
      stop(
        "If provided, `mus` should be a list of length equal to that of ",
  ...
  counts <- do.call(
    cbind,
    lapply(seq_along(mus), FUN = function(i) {
      matrix(rpois(ncells[[i]] * ngenes, mus[[i]]), nrow = ngenes)
    })
  )
  sce <- SingleCellExperiment(
    list(counts = counts),
    colData = data.frame(type = "singlet", origin = rep(names(ncells), ncells))
  )

  # doublets
  expected <- getExpectedDoublets(
    sce$origin,
    dbl.rate,
  ...
  if (sum(simdbl) > 0) {
    mus <- lapply(
      strsplit(names(expected), "+", fixed = TRUE),
      FUN = function(x) {
        mus[[x[[1]]]] + mus[[x[[2]]]]
      }
    )
...
.checkProcArg <- function(processing) {
  ...
  if (is.character(processing)) {
    ...
    processing <- match.arg(
      processing,
      c("default", "rawPCA", "rawFeatures", "normFeatures", "atac")
    )
    ...
    stop(
      "If a function, `processing` should have at least the arguments `e` ",
...
.clustSpearman <- function(e, clusters, nMarkers = 30) {
  if (is.null(dim(clusters))) {
    ...
    if (nMarkers > 0 & nMarkers < nrow(e)) {
      suppressWarnings(
        mm <- scran::findMarkers(e2, groups = clusters, test.type = "binom")
      )
      g <- unique(unlist(lapply(
        mm,
        FUN = function(x) row.names(x)[seq_len(nMarkers)]
      )))
    }
    ...
    if (ncol(clusters) > 500)
      warning(
        "You're using a very large `clustCor` matrix, are you sure that the",
        "columns are cell types, and not individual cells?"
      )
    g <- intersect(row.names(e), row.names(clusters))
    if (length(g) < 5) {
      warning(
        "Too few marker genes in common between `clustCor` and the data.",
...
.checkSCE <- function(sce, coerce = TRUE) {
  ...
  if (min(colSums(counts(sce))) < 200)
    warning(
      "Some cells in `sce` have an extremely low read counts; note ",
  ...
  if (is.null(row.names(sce))) row.names(sce) <- paste0("f", seq_len(nrow(sce)))
  sce
}
...
.rescaleSampleScores <- function(
  ...
  if (is.null(q)) {
    if (byType) {
      ...
      q <- t(vapply(
        split(d[[what]], d$sample),
        FUN.VALUE = numeric(2L),
        FUN = function(x) {
          sort(c(
            mean(x[-wD], na.rm = TRUE, trim = 0.5),
            mean(x[wD], na.rm = TRUE, trim = 0.5)
          ))
        }
      ))
  ...
  if (ncol(q) > 1) {
    ...
    d[[newName]][w] <- qmeds[1] +
      .qscale(or.sc[w], by = d$sample[w]) * (qmeds[2] - qmeds[1])
    w <- which(or.sc > q[d$sample, 2])
    d[[newName]][w] <- qmeds[2] +
      .qscale(or.sc[w], by = d$sample[w]) * (1 - qmeds[2])
    maxs <- vapply(
      split(d[[newName]], d$sample),
      FUN.VALUE = numeric(1),
      na.rm = TRUE,
      FUN = max
    )
...
directDblClassification <- function(
  sce,
  dbr = NULL,
  processing = "default",
  iter = 2,
  ...
  for (i in seq_len(iter)) {
    dT <- suppressWarnings(doubletThresholding(
      d,
      dbr = dbr,
      returnType = "call"
    ))
