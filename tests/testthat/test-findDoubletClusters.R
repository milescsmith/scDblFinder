...
test_that("findDoubletClusters agrees with a reference implementation", {
  ...
  clusters <- rep(
    1:4,
    c(ncol(counts.1), ncol(counts.2), ncol(counts.3), ncol(counts.m))
  )

  dbl <- findDoubletClusters(counts, clusters, get.all.pairs = TRUE)
  ref <- scran::findMarkers(
    scuttle::normalizeCounts(counts),
    clusters,
    full.stats = TRUE
  )
  ...
  for (x in rownames(dbl)) {
    ...
    collected <- apply(combos, 2, function(chosen) {
      ...
      data.frame(
        best = rownames(stats)[which.min(p)],
        p.val = min(adj.p),
    ...
    expect_identical(
      sort(c(dbl[x, "source1"], dbl[x, "source2"])),
      sort(combos[, to.use])
    )
  }
})
...
test_that("findDoubletClusters works correctly with row subsets", {
  ...
  expect_error(
    findDoubletClusters(counts[, 0], clusters[0]),
    "need at least three"
  )
})
