...
test_that("computeDoubletDensity PC spawning works correctly", {
  ...
  set.seed(12345)
  sim.pcs <- scDblFinder:::.spawn_doublet_pcs(
    counts,
    sf,
    SVD$v,
    centers,
  ...
  set.seed(23456)
  ref1 <- scDblFinder:::.spawn_doublet_pcs(
    counts,
    sf,
    SVD$v,
    centers,
  ...
  ref2 <- scDblFinder:::.spawn_doublet_pcs(
    counts,
    sf,
    SVD$v,
    centers,
  ...
  ref3 <- scDblFinder:::.spawn_doublet_pcs(
    counts,
    sf,
    SVD$v,
    centers,
...
test_that("size factor variations in computeDoubletDensity work correctly", {
  ...
  set.seed(12345)
  ref <- computeDoubletDensity(
    counts,
    size.factors.norm = scuttle::librarySizeFactors(counts)
  )
  ...
  set.seed(23456)
  out <- computeDoubletDensity(
    t(t(counts) / sf1),
    size.factors.norm = rep(1, ncol(counts)),
    size.factors.content = 1 / sf1
  )
  expect_equal(out, ref)
  ...
  out <- computeDoubletDensity(
    scaled,
    size.factors.norm = prod,
    size.factors.content = sf2
  )
  expect_equal(out, ref)
