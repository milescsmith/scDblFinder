...
test_that("fastcluster works as expected", {
  ...
  expect_gt(
    sum(apply(table(sce$cluster, sce$fastcluster), 1, max)[1:4]) /
      sum(sce$type == "singlet"),
    0.8
  )
  x <- fastcluster(
    sce,
  ...
  expect_gt(
    sum(apply(table(sce$cluster, x), 1, max)[1:3]) /
      sum(sce$type == "singlet"),
    0.8
  )
})
...
test_that("scDblFinder works as expected", {
  ...
  expect(
    min(sce$scDblFinder.score) >= 0 & max(sce$scDblFinder.score) <= 1,
